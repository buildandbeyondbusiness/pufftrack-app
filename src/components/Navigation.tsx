...
import { Flame, Clock, BarChart3, User } from 'lucide-react';
import { soundManager } from '../utils/audio';
import { usePuff } from '../context/PuffContext';

export type TabType = 'home' | 'history' | 'progress' | 'profile';
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center rounded-full px-4 py-2 transition-all duration-300 ${
                isActive
            ...
            >
              ...
              {tab.id === 'profile' && user?.photoURL ? (
                <img
                  src={user.photoURL}
                  alt="Avatar"
                  className={`relative z-10 h-5 w-5 rounded-full border ${isActive ? 'border-[var(--accent-purple)] ring-2 ring-[var(--accent-purple-glow)]' : 'border-transparent'}`}
                />
              ) : (
                <Icon
